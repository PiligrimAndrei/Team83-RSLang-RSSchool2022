...
export class GameSprint extends Component {
  ...
  private async getWordsForGame(group: number, page: number): Promise<void> {
    ...
    if (data) {
      ...
      let guessWord = this.getRandomTranslateWord(wordArr).word
      let guessTranslateWord = this.getRandomTranslateWord(wordArr).wordTranslate
      this.renderWords(guessWord, guessTranslateWord)
    }
  }
  private renderWords(wordC: string, wordT: string) {
  ...
  private getRandomTranslateWord(arr: IWord[]) {
    const random = Math.round(Math.random() * arr.length);
    return arr[random]
  }

  private submitYes() {
    console.log(this.totalScore, this.scoreWord)
    if (this.isValid) {
